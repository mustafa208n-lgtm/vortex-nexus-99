...
# --- Reference Toggle (Placed at the top of the Sidebar for instant response) ---
show_reference = st.sidebar.checkbox("📖 View All 49 Ideas & Boxes Reference")

if show_reference:
    st.markdown("---")
    st.header("📋 Comprehensive Reference Guide to All 49 Ideas in AutoVolt AI")
    
    st.subheader("Box 1: Sensors & Hydraulic Presses")
    st.markdown("- **Point 1:** Live hydraulic pressure sensor in bar.")
    st.markdown("- **Point 2:** Iron press vibration and oscillation detection sensor.")
    st.markdown("- **Point 3:** Central engine temperature sensor.")
    st.markdown("- **Point 4:** Manual human load control slider.")
    st.markdown("- **Point 5:** Cumulative operating hours counter.")
    st.markdown("- **Point 6:** Worker fatigue and distraction estimator (aligned with the European AI Act).")

    st.subheader("Box 2: Design UI & Silent Alarm System")
    st.markdown("- **Point 7:** Zen Industrial Style calm European aesthetic design.")
    st.markdown("- **Point 8:** Clean, unified KPI digital cards for commission cash and budget display.")
    st.markdown("- **Point 9:** Strict red flashing alarm wall protected by update lock to prevent freezing.")
    st.markdown("- **Point 10:** Silent triple-signal system to summarize plant status at a single glance.")
    st.markdown("- **Point 11:** Night shift auto-magnification mode.")
    st.markdown("- **Point 12:** Fail-Closed safety shutdown system.")

    st.subheader("Box 3: AI Algorithms & Micro-SaaS Services")
    st.markdown("- **Point 13:** Risk training and prediction engine via RandomForest algorithm.")
    st.markdown("- **Point 14:** Micro-SaaS fractional subscription packages.")
    st.markdown("- **Point 15:** Backend server decoupling and API interface display.")
    st.markdown("- **Point 16:** Encrypted JSON REST API load display screen.")
    st.markdown("- **Point 17:** Mosquitto MQTT message broker worker.")
    st.markdown("- **Point 18:** Non-blocking background queue for logistics data processing and feeding.")

    st.subheader("Box 4: Cyber Locks & Forensic Audit Security")
    st.markdown("- **Point 19:** Lock blocking data wipe and forensic tampering to clear tax liability.")
    st.markdown("- **Point 20:** European Sovereign Identity Vault for 2FA verification.")
    st.markdown("- **Point 21:** Account linking lock bound to IP address and browser fingerprint.")
    st.markdown("- **Point 22:** Mutual physical sensor encryption lock (mTLS Handshake).")
    st.markdown("- **Point 23:** Role-Based Access Control (RBAC) lock.")
    st.markdown("- **Point 24:** Firewall lock against cyber flooding and organized DDoS.")
    st.markdown("- **Point 25:** Payout withdrawal link protection lock via renewing HMAC-SHA256 algorithms.")
    st.markdown("- **Point 26:** 3D biometric or SMS-based verification system.")
    st.markdown("- **Point 27:** Cross-border encrypted cloud migration and backup lock.")
    st.markdown("- **Point 28:** Global atomic time synchronization via unified time function.")
    st.markdown("- **Point 29:** Strict library version freezing lock to block compromised updates.")
    st.markdown("- **Point 30:** Pre-isolated financial escrow lock for contract accounts to protect against bankruptcy.")

    st.subheader("Box 5: Marketplace, Commission Bank & Transfers")
    st.markdown("- **Point 31:** Automated fund splitting and instant commission transfer engine via payment gateway.")
    st.markdown("- **Point 32:** One-touch instant logistics contracting cards between workshops and factories.")
    st.markdown("- **Point 33:** Factory central treasury budget tracking dashboard.")
    st.markdown("- **Point 34:** Real-time treasury balance protection and updates.")
    st.markdown("- **Point 35:** Tax isolation wall and automatic tax clearance from European VAT issues.")
    st.markdown("- **Point 36:** Encrypted forensic audit log table.")
    st.markdown("- **Point 37:** SHA-256 fingerprinting for signing audit events and tamper protection.")
    st.markdown("- **Point 38:** Timeseries data storage and retrieval system.")
    st.markdown("- **Point 39:** Encrypted CAD engineering blueprint storage archive.")
    st.markdown("- **Point 40:** Ready technical evidence collector for auditors and environmental inspectors.")

    st.subheader("Box 6: Carbon Engine, Stock Exchange & Eco-Arbitrage")
    st.markdown("- **Point 41:** Carbon index and regional environmental tax calculation engine.")
    st.markdown("- **Point 42:** Lightweight cloud digital twin programmed with SVG code.")
    st.markdown("- **Point 43:** Air-Lock lock to restrict crane and equipment operation at night.")
    st.markdown("- **Point 44:** Real-time energy stock tracking and financial arbitrage engine (Energy Hub).")
    st.markdown("- **Point 45:** Generative AI integration via Mistral model and restricted RAG manuals.")
    st.markdown("- **Point 46:** Strict mathematical character and hyper-data filter to prevent hallucinations.")

    st.subheader("Box 7: Workshops & Logistics Commission Deductions")
    st.markdown("- **Point 47:** Geo-search and mapping engine for vacant square meters in European cities.")
    st.markdown("- **Point 48:** Serial-linked rare industrial spare parts and tools inventory and exchange system.")
    st.markdown("- **Point 49:** Isolated automated logistics commission deduction engine routing funds directly to your phone in Samawah, Iraq.")
